...
def main() -> None:
    ...
    try:
        ...
        for krithi_id, _title in krithis_with_gaps:
            ...
            for variant_id, _language, _script, lyrics in variants_with_gaps:
                ...
                for section_id, section_type, order_index in canonical_sections:
                    ...
                    if texts:
                        text = texts.pop(0)
                    elif not result.sections or (
                        len(result.sections) == 1 and result.sections[0].section_type.name == "OTHER"
                    ):
