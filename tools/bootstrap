...
TEMPLATE="$ROOT_DIR/tools/bootstrap-assets/env/development.env.example"
TARGET="$ROOT_DIR/config/development.env"

if [[ -f "$TARGET" ]]; then
  printf "config/development.env already exists (leaving as-is)\n"
else
  if [[ -f "$TEMPLATE" ]]; then
    cp "$TEMPLATE" "$TARGET"
    printf "Created config/development.env from template\n"
  else
