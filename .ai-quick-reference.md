# AI Context Quick Reference Card - Sangeetha Grantha Platform

## 🚀 Quick Start

| AI Tool | Context File | How to Use |
|---------|-------------|------------|
| **ChatGPT** | `.chatgpt-config.md` | "Read .chatgpt-config.md and help me..." |
| **Cursor** | `.cursorrules` | Auto-loaded, use Cmd/Ctrl+K |
| **Claude** | `.claude-context.md` | "Using .claude-context.md, please..." |
| **Gemini** | `.gemini-context.md` | "Reference .gemini-context.md to..." |
| **Windsurf** | `.windsurf/context.md` | Auto-loaded in Windsurf |
| **Aider** | `.aiderignore` | Auto-respected, run `aider` |
| **Antigravity** | `.antigravity.md` | "Read .antigravity.md and help me..." |

## 🎯 Key Facts

### Tech Stack
```
Kotlin:     2.2.20    (Multiplatform, JVM, Android)
React:      19.2.0    (TypeScript admin web)
TypeScript: 5.8.3     (Strict mode)
Ktor:       3.3.1     (Backend API)
Exposed:    1.0.0-rc-2 (ORM DSL)
PostgreSQL: 15+       (Database)
Vite:       7.1.7     (Build tool)
Tailwind:   3.4.13    (Styling)
```

### Module Structure
```
modules/
├── shared/domain/           # Common DTOs (KMP)
├── backend/
│   ├── api/                 # Ktor routes
│   └── dal/                 # Exposed ORM
└── frontend/
    └── sangita-admin-web/     # React TS (ACTIVE)
```

### Database
- **Tool**: Rust CLI (`database/rust/`)
- **NOT**: Flyway ❌
- **Run**: `cd database/rust && cargo run migrate`

## ⚠️ Critical Rules

### Backend (Kotlin)
```kotlin
// ✅ DO: Use DatabaseFactory
suspend fun getSangita(id: UUID) = DatabaseFactory.dbQuery {
    SangitasTable.selectAll().where { id eq id }
}

// ✅ DO: Return DTOs
data class Sangita(val id: UUID, val name: String)

// ✅ DO: Log mutations
AuditLogTable.insert { /* log */ }

// ❌ DON'T: Skip dbQuery
// ❌ DON'T: Return Exposed entities
// ❌ DON'T: Skip audit logs
```

### Frontend (TypeScript)
```typescript
// ✅ DO: Explicit types
interface Props { sangita: Sangita; }

// ✅ DO: Tailwind classes
<div className="rounded-lg border p-4">

// ✅ DO: Functional components
export const SangitaCard: React.FC<Props> = ({ sangita }) => {

// ❌ DON'T: Use any
// ❌ DON'T: Skip prop types
```

## 📋 Common Commands

```bash
# Full Stack Dev
cd tools/sangita-cli && cargo run -- dev --start-db

# DB Reset
cd tools/sangita-cli && cargo run -- db reset

# Android
./gradlew :androidApp:installDebug

# Versions
cat gradle/libs.versions.toml
```

## 🚫 Never Do

1. ❌ Suggest Flyway (use Rust migrations)
3. ❌ Use `any` in TypeScript
4. ❌ Skip `DatabaseFactory.dbQuery`
5. ❌ Return Exposed entities
6. ❌ Add deps without version catalog

## 🛠 Tooling Updates (Dec 2025)

**Sangita CLI (`tools/sangita-cli/`)**
- v0.1.0 bundles setup/reset/dev/test flows (released 2025-11-27).
- `cargo run -- dev --start-db` boots DB → backend → React admin for live smoke checks.
- `cargo run -- test steel-thread` resets DB, seeds `test_data.json`, drives create/import/dashboard/payment flows, and leaves services up for manual QA.



## ✅ Always Do

1. ✅ Use `DatabaseFactory.dbQuery { }`
2. ✅ Log to `AUDIT_LOG`
3. ✅ Return DTOs
4. ✅ Reference `gradle/libs.versions.toml`
5. ✅ Use TypeScript strict mode
6. ✅ Use Tailwind utilities

## 📚 Documentation

| Need | See |
|------|-----|
| Backend patterns | `application_documentation/backend/architecture.md` |
| Tech stack | `application_documentation/tech-stack.md` |
| API contracts | `application_documentation/api/integration-spec.md` |
| Database schema | `application_documentation/database/schema.md` |
| Full guide | `AI-CONTEXT-README.md` |

## 🔍 Troubleshooting

| Problem | Solution |
|---------|----------|
| AI suggests Flyway | "This project uses Rust migrations only" |
| AI uses `any` | "Use TypeScript strict mode, no any" |
| AI skips dbQuery | "All DB ops must use DatabaseFactory.dbQuery" |

## 💡 Pro Tips

1. **Start conversations** by referencing the context file
2. **Be specific** about which patterns to follow
3. **Test locally** before asking AI to commit
4. **Check versions** in `gradle/libs.versions.toml`
5. **Read docs** in `application_documentation/`

## 🎨 Code Patterns

### Ktor Route
```kotlin
fun Route.sangitaRoutes(service: SangitaService) {
    authenticate("jwt") {
        get("/v1/admin/sangitas") {
            call.respond(service.getAll())
        }
    }
}
```

### React Component
```typescript
interface SangitaCardProps {
  sangita: Sangita;
  onEdit?: (id: string) => void;
}

export const SangitaCard: React.FC<SangitaCardProps> = 
  ({ sangita, onEdit }) => (
    <div className="rounded-lg border p-4">
      <h3 className="font-semibold">{sangita.name}</h3>
      {onEdit && <button onClick={() => onEdit(sangita.id)}>Edit</button>}
    </div>
  );
```

### Database Query
```kotlin
suspend fun findById(id: UUID): Sangita? = 
  DatabaseFactory.dbQuery {
    SangitasTable
      .selectAll()
      .where { SangitasTable.id eq id }
      .map { it.toSangita() }
      .singleOrNull()
  }
```

---

**Print this card** or keep it handy when working with AI assistants!

For full details, see:
- 📖 `AI-CONTEXT-README.md` - Complete guide
- 📊 `CONTEXT-FILES-SUMMARY.md` - What changed
- 🗂️ `.ai-context-guide.md` - File comparison
