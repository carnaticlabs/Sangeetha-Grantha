# AI Context Quick Reference Card - Sangeetha Grantha Platform

## 🚀 Quick Start

| AI Tool | Context File | How to Use |
|---------|-------------|------------|
| **ChatGPT** | `.chatgpt-config.md` | "Read .chatgpt-config.md and help me..." |
| **Cursor** | `.cursorrules` + `.cursor/rules/` | Auto-loaded; skills/commands mirror `.claude/` |
| **Claude** | `CLAUDE.md` | Canonical rules; `.claude-context.md` is a thin pointer |
| **Gemini** | `.gemini-context.md` | "Reference .gemini-context.md to..." |
| **Aider** | `.aiderignore` | Auto-respected, run `aider` |
| **Antigravity** | `.agents/AGENTS.md` | "Read .agents/AGENTS.md and help me..." |

## 🎯 Key Facts

### Tech Stack & Versions
For current toolchain and library versions, see **[Current Versions](application_documentation/00-meta/current-versions.md)**.

### Module Structure

```mermaid
flowchart TB
  subgraph repo["modules"]
    direction TB

    subgraph shared["shared"]
      direction TB
      DOM[domain]
      PRES[presentation]
      MDATA[mobile-data]
    end

    subgraph mobile["mobile"]
      direction TB
      AND[androidApp]
      IOS[iosApp]
    end

    subgraph backend["backend"]
      direction TB
      API[api]
      DAL[dal]
    end

    WEB[frontend/sangita-admin-web]
  end
```

### Database
- **Tool**: **Flyway** (ADR-013), driven via the Makefile
- **NOT**: Liquibase, custom runners, the archived Rust CLI, or the superseded Python `db-migrate`
- **Run**: `make migrate` (or `make db-reset`)

## ⚠️ Critical Rules

### Backend (Kotlin)
```kotlin
// ✅ DO: Use DatabaseFactory.dbQuery
suspend fun getKrithi(id: UUID) = DatabaseFactory.dbQuery {
    KrithisTable.selectAll().where { id eq id }
}

// ✅ DO: Return DTOs
data class KrithiDto(val id: UUID, val title: String)

// ✅ DO: Log mutations
dal.auditLogs.append { /* log */ }

// ❌ DON'T: Skip dbQuery
// ❌ DON'T: Return Exposed entities
// ❌ DON'T: Skip audit logs
```

### Frontend (TypeScript)
```typescript
// ✅ DO: Explicit types
interface Props { krithi: KrithiDto; }

// ✅ DO: Tailwind classes
<div className="rounded-lg border p-4">

// ✅ DO: Functional components with hooks
export const KrithiCard: React.FC<Props> = ({ krithi }) => {

// ❌ DON'T: Use any
// ❌ DON'T: Skip prop types
```

## 📋 Common Commands

```bash
make dev                 # DB + backend + frontend + extraction
make db-reset            # drop → create → Flyway migrate
make test                # backend unit
make test-integration    # Testcontainers
make test-frontend       # Curator Console
make test-mobile         # Rasika JVM
make mobile-android      # assembleDebug
make check-docs
```

## 🚫 Never Do

1. ❌ Suggest Liquibase or a custom migration runner (this project uses **Flyway**, ADR-013)
2. ❌ Invoke the archived Rust CLI or the superseded Python `db-migrate`
3. ❌ Use `any` in TypeScript
4. ❌ Skip `DatabaseFactory.dbQuery`
5. ❌ Return Exposed entities
6. ❌ Add deps without version catalog

## ✅ Always Do

1. ✅ Use `DatabaseFactory.dbQuery { }`
2. ✅ Log to `audit_log`
3. ✅ Return DTOs
4. ✅ Reference `gradle/libs.versions.toml`
5. ✅ Use TypeScript strict mode
6. ✅ Use Tailwind utilities

## 📚 Documentation

| Need | See |
|------|-----|
| Backend patterns | `application_documentation/02-architecture/backend-system-design.md` |
| Tech stack | `application_documentation/02-architecture/tech-stack.md` |
| API contracts | `application_documentation/03-api/api-contract.md` |
| Database schema | `application_documentation/04-database/schema.md` |
| Getting Started | `application_documentation/00-onboarding/getting-started.md` |

## 🔍 Troubleshooting

| Problem | Solution |
|---------|----------|
| AI suggests Liquibase / custom runner | "This project uses Flyway only (ADR-013), via `make migrate`" |
| AI uses `any` | "Use TypeScript strict mode, no any" |
| AI skips dbQuery | "All DB ops must use DatabaseFactory.dbQuery" |

## 💡 Pro Tips

1. **Start conversations** by referencing the context file
2. **Be specific** about which patterns to follow
3. **Test locally** before asking AI to commit
4. **Check versions** in `gradle/libs.versions.toml`
5. **Read docs** in `application_documentation/`

## 🎨 Code Patterns

### Ktor Route
```kotlin
fun Route.krithiRoutes(service: IKrithiService) {
    authenticate("jwt") {
        get("/v1/admin/krithis") {
            call.respond(service.search(request))
        }
    }
}
```

### React Component
```typescript
interface KrithiCardProps {
  krithi: KrithiDto;
  onEdit?: (id: string) => void;
}

export const KrithiCard: React.FC<KrithiCardProps> = 
  ({ krithi, onEdit }) => (
    <div className="rounded-lg border p-4">
      <h3 className="font-semibold">{krithi.title}</h3>
      {onEdit && <button onClick={() => onEdit(krithi.id)}>Edit</button>}
    </div>
  );
```

---

**Print this card** or keep it handy when working with AI assistants!