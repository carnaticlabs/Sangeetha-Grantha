...
| T55.3 | Implement left-side mātrā reordering and vowel merging | ि after consonant in output; a + aa-mātrā → आ where applicable. | `tools/krithi-extract-enrich-worker/src/velthuis_decoder.py` |
...
| File | Change |
|:---|:---|
| `tools/krithi-extract-enrich-worker/src/velthuis_decoder.py` | New — encoding extraction, byte→Unicode decode, reordering/merging |
| `tools/krithi-extract-enrich-worker/tests/test_velthuis_decoder.py` | New — unit tests |
| `tools/krithi-extract-enrich-worker/src/extractor.py` | Detect Velthuis fonts, decode spans |
| `tools/krithi-extract-enrich-worker/src/worker.py` | language/script for Sanskrit; optional titleLatin |
| `tools/krithi-extract-enrich-worker/src/schema.py` | Optional titleLatin in CanonicalExtractionDto |
