...
- [x] Repointed `IntegrationTestBase` at the new substrate; between tests, transactional tables are truncated and the reference tables (composers/ragas/talas/deities/import_sources) are **reset to a post-migration seed snapshot** (test-created rows deleted, seeded rows untouched) — seed read-only, suite idempotent against a persistent/external DB (verified by a double run via `TEST_DATABASE_URL`). *(Upgraded from the interim schema-only approach; addresses code-review feedback on reference-table mutation.)*
