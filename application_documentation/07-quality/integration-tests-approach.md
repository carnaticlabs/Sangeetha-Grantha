...
**Step 3 — DAL suite (2–4 days).** Stand up `dal/src/test` with scenarios D1–D6. D1 (migrations-from-scratch) alone is the cheapest high-value test in the entire plan and also satisfies north-star Phase 0 item 3's "apply all migrations to a scratch Postgres" CI check — and, post-Step 2, additionally verifies the Flyway cutover.

**Step 4 — CI activation (1 day, with north-star Phase 0).** GitHub Actions: `unit → integrationTest → frontend typecheck+build → worker pytest`. Testcontainers needs zero special configuration on hosted runners. Every test written in Steps 1–3 becomes a gate the moment this lands.
...
- **Revisit when:** pgvector lands (image swap), CI matures (consider service-container mode for speed via the escape hatch), canonical revisioning (N5) adds bitemporal scenarios to §4, or the Phase 3 deployment work makes the Podman spike (§3.5) worth running.
