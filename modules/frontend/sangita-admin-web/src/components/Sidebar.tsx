...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate }) => {
  ...
  const isCurrent = (id: ViewState) => {
    if (id === ViewState.KRITHIS && currentView === ViewState.KRITHI_DETAIL) return true;
      ...
      <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-200 group ${
              isCurrent(item.id)
          ...
          >
            <span
              className={`material-symbols-outlined text-2xl ${
                isCurrent(item.id) ? 'text-primary fill-1' : 'text-ink-500 group-hover:text-primary transition-colors'
              }`}
            >
              {item.icon}
            </span>
