...
- **Headers**: Maintain `Status`, `Version`, `Last Updated` headers in all markdown files.
- **Links**: Use relative links only. Verify they work.
- **Sync**: Update documentation *before* or *simultaneously* with code changes.

### Migrations
1. **MUST use Rust migration tool** - located in `tools/sangita-cli/`
2. **NEVER suggest Flyway** - project explicitly does NOT use it
3. **Migration workflow**:
   - Create SQL file: `database/migrations/V###__description.sql`
   - Run: `cd tools/sangita-cli && cargo run -- db migrate`
