# AI Context Files Guide - Sangeetha Grantha Platform
...
### 8. `GOOSE.md`
**For**: Goose AI
**Format**: Markdown with structured technical rules
**Purpose**: Comprehensive technical and musicological context for Goose
**Best For**: Full-stack development, enforcement of complex domain rules

**Structure**:
- Persona & Context
- Critical Technical Rules (DB, Conductor, Commits)
- Architecture (Backend, Frontend, Shared)
- Musicological Domain Rules
- Response Style Standards

### 8. `GOOSE.md` and `.goose-context.md`
**For**: Goose AI (Block/Square/CashApp/Tidal)
**Format**: Markdown with structured sections
**Purpose**: Comprehensive project configuration tailored for Goose AI agent
**Best For**: General coding assistance, tool calling workflows, and multi-model support

**Files**:
- `GOOSE.md`: Concise guide with persona, critical rules, and response style guidelines
- `.goose-context.md`: Detailed context file with comprehensive patterns, examples, and reference material

**Structure**:
- Identity & Persona (dual-expert: Musicologist + Architect)
- Project Context & System Architecture
- Critical Technical Rules (Database, Conductor, Commit Guardrails)
- Development Patterns (Backend, Frontend, Shared Domain)
- Musicological Domain Rules
- Response Style Guidelines (Markdown formatting, code blocks)
- Tooling Updates & Common Commands
...
| Tool calling workflows | `GOOSE.md` / `.goose-context.md` |
...
2. **Track File**: Maintain `conductor/tracks/TRACK-<ID>-<slug>.md`.
3. **Progress**: Update "Progress Log" in the track file.

### Commit Guardrails
1. **Ref**: All commits must Ref a doc.
2. **Atomic**: One task per commit.
3. **No Secrets**: Scan before commit.

### Documentation Standards
1. **Headers**: STRICT Markdown Table required (Status/Version/Last Updated/Author).
2. **Links**: Relative links only.
...
| `GOOSE.md` / `.goose-context.md` | Quarterly | Medium |
...
# Database Operations
cd tools/sangita-cli && cargo run -- db reset
