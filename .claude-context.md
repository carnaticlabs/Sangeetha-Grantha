...
> Cursor/Copilot setups) is a thin pointer to it. Current Cursor config is
> [`.cursorrules`](.cursorrules), [`.cursor/rules/`](.cursor/rules/), and
> [`.cursor/skills/`](.cursor/skills/) (symlinks to `.claude/skills/`). All guidance —
> tech stack, architecture, the migration engine, audit-log and commit rules, ports,
> key docs, and the specialist subagents — lives in `CLAUDE.md`. It does **not** restate
> shared rules here, so it can't drift.
